...
| 23 | Single tool registry — Medium | `src/_data/tools.js`, `src/tools.njk`, `src/index.njk`, `src/sitemap.njk`, `src/js/app.js`, generated registry | Registry now drives directory, sitemap, search, related links, navigation visibility, canonical metadata inputs, homepage featured tools, and drag-and-drop recommendations. | Fixed locally; a new tool should be added to the registry and then built/synced. |
